...
class DesktopState extends MusicBeatState
{
				...
				case 1:
					...
					for (i in songs) {
						if (songsUnlocked[songs.indexOf(i)]) { 
							...
							#if sys
							if (sys.FileSystem.exists(Paths.instEXcheck(i))) {
								theText += '&-& %ex% &score:& %' + Highscore.getScore(songHighscore, 3) + '%\n';
								if (Highscore.getMissesString(songHighscore, 3) == '0')
									theText += '&-& %ex% &misses:& *Perfect!*\n';
								else
	...
	override function update(elapsed:Float)
	{
		...
		androidback = FlxG.android.justReleased.BACK;
		if (theSong != null)
			theSong.volume = FlxG.sound.volume;
		FlxG.watch.addQuick("menu: ", currentMenu);	
		FlxG.watch.addQuick("closingFolder: ", closingFolder);
		for (i in folders) {
			/*for (touch in FlxG.touches){
				if (touch.overlaps(i) && touch.pressed){
					i.isDragged = true;
					draggingFolder = true;
				}
				if (touch.justReleased && (draggingFolder || i.isDragged)){
						i.isDragged = false;
						draggingFolder = false;
						centerFolder(i,FlxG.mouse.x,FlxG.mouse.y);
					}
			}*/
			for (touch in FlxG.touches.list){
				if (touch.overlaps(i) && !i.isDragged) {
					var doStuff:Bool = false;
					if (folderDrag.visible == true) {
						if (!FlxG.mouse.overlaps(folder.members[0]))
							doStuff = true;
					} else if (notepadDrag.visible == true) {
						if (!touch.overlaps(notepad.members[0]))
							doStuff = true;
					} else doStuff = true;
		...
		for (touch in FlxG.touches.list){
			if (!touch.overlaps(folders) || draggingFolder || touch.overlaps(folder.members[0]) && folderDrag.visible == true || touch.overlaps(notepad.members[0]) && notepadDrag.visible == true)
				folderBorder.visible = false;
		}
		if (draggingFolder) {
			for (i in 0...folders.length) {
		...
		if (notepadDrag.visible) {
			if (androidback && canOpenNotepad && currentMenu == 'notepad') {
				closeNotepad();
				
			}
			var exitButton = notepad.members[notepad.length - 1];
			for (touch in FlxG.touches.list){
				if (touch.overlaps(notepad.members[notepad.length - 1])) {
					exitButton.color = FlxColor.fromHSL(exitButton.color.hue, exitButton.color.saturation, 1, 1);
					if (canOpenNotepad && currentMenu == 'notepad' && touch.justPressed) {
						closeNotepad();
						closingFolder = true;
					}
				} else {
			...
			for (touch in FlxG.touches.list){
				if (FlxG.mouse.y > 54 + notepad.members[0].y && touch.overlaps(notepad.members[0])) {
					var doStuff:Bool = false;
					if (folderDrag.visible && currentMenu == 'freeplay') {
						if (!touch.overlaps(folder.members[0]))
							doStuff = true;
						} else
							doStuff = true;
				...
				if (currentMenu == 'notepad' && folderDrag.visible == true && !touch.overlaps(notepad.members[0]) && touch.overlaps(folder.members[0]) && touch.justPressed) {
				remove(notepad);
				remove(folder);
				remove(freeplayStuff);
				remove(freeplayLocks);
		...
		if (folderDrag.visible) {
			if (androidback && canOpenFolder && currentMenu == 'freeplay') {
				closeFolder();
				closingFolder = false;
			}
			for (touch in FlxG.touches.list){
				if (currentMenu == 'freeplay' && notepadDrag.visible == true && touch.overlaps(notepad.members[0]) && !touch.overlaps(folder.members[0]) && touch.justPressed) {
					remove(notepad);
					remove(folder);
					remove(freeplayStuff);
					remove(freeplayLocks);
				...
				for (i in freeplayStuff) {
					if (touch.overlaps(i)) {
						...
						if (runShit) {
							...
							folderSelect.y = i.y - 24;
							folderSelect.visible = true;
							if (touch.justPressed && songsUnlocked[freeplayStuff.members.indexOf(i)]) {
								var diffState:DifficultySelectSubstate = new DifficultySelectSubstate(songs[freeplayStuff.members.indexOf(i)]);
								diffState.closeFunction = function() {
									closingFolder = true;
								};
				...
				if (!touch.overlaps(freeplayStuff))
					folderSelect.visible = false;
				if (folderDrag.isDragged) {
					for (i in folder) {
						i.setPosition(folderDrag.x + folderOffsets[folder.members.indexOf(i)].x, folderDrag.y + folderOffsets[folder.members.indexOf(i)].y);
